...
export const generatePrescriptionPDF = async (prescription, patient, doctor) => {
  try {
    ...
    pdfService.addHeader(doc, 'Medical Prescription', {
      subtitle: 'Clinical document generated by MediQueue',
      contactLines: [
...
export const generateMedicalRecordPDF = async (record, patient, uploader) => {
  try {
    ...
    const qrY = doc.y + 5;
    await pdfService.addQRCode(doc, `${process.env.FRONTEND_URL || 'https://mediqueue.com'}/records/${record._id}`, 50, qrY, 80);
    doc.y = qrY + 90;
    doc.moveDown(1);
...
export const generateAppointmentPDF = async (appointment, patient, doctor) => {
  try {
    ...
    doc.fontSize(8.5)
      ...
      .fillColor('#7C2D12')
      .text(
        'Cancellation policy: Please cancel or reschedule at least 24 hours in advance to avoid cancellation charges. For assistance, contact +91-XXX-XXX-XXXX or info@mediqueue.com.',
        62,
        policyY + 10,
        {
          width: panelWidth - 24,
